...
Targets the dedicated spine pipeline database (SPINE_DB, defaulting to the
local spine_db on port 57422) rather than the shared DATABASE_URL default, so
this test can run alongside other local pipeline databases without clobbering
their state.
"""
