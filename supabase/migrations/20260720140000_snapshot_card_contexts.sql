...
-- Both pipeline namespaces capture the same live clustering surface. Keep one
-- builder so schema additions cannot silently diverge between experiment
-- families. Schema v2 adds exact card contexts and source identity/hashes.
create function public.build_current_cluster_snapshot_v2()
returns jsonb
language sql
stable
security definer
    ...
    select pg_catalog.jsonb_build_object(
        ...
        'episodes', coalesce((
            select pg_catalog.jsonb_agg(
                pg_catalog.to_jsonb(row_data) order by row_data.first_entry_at, row_data.id
            )
            from (
                select id, storyline_id, status, centroid, entity_set, event_keys,
                       entry_count, first_entry_at, newest_entry_at, attach_method,
                       attach_similarity, attach_reason, adjudicator_model, created_at
                from public.episodes
            ) row_data
        ), '[]'::jsonb),
        'episode_entries', coalesce((
            select pg_catalog.jsonb_agg(
                pg_catalog.to_jsonb(row_data) order by row_data.episode_id, row_data.entry_id
            )
            from (
        ...
        'news_entries', coalesce((
            select pg_catalog.jsonb_agg(
                pg_catalog.to_jsonb(row_data) order by row_data.published_at, row_data.id
            )
            from (
                select ne.id, ne.news_source_id, ne.title, ne.summary, ne.url,
                       ne.url_canonical, ne.content_hash, ne.published_at,
                       ne.entity_set, ne.event_keys, nsp.publisher_key as agency
        ...
        'event_cards', coalesce((
            select pg_catalog.jsonb_agg(
                ...
                order by row_data.storyline_id, row_data.kind, row_data.version desc
            )
            from (
        ...
        'event_card_contexts', coalesce((
            select pg_catalog.jsonb_agg(
                pg_catalog.to_jsonb(row_data) order by row_data.event_card_id
            )
            from (
                select event_card_id, storyline_id, snapshot_schema_version,
                       knowledge_cutoff_at, source_run_id, capture_method,
                       source_entry_ids, source_content_hashes, episode_ids,
                       first_entry_at, newest_entry_at, entry_count,
                       original_entry_count, syndicated_entry_count, episode_count,
                       agency_ids, news_source_ids, distinct_feeds,
                       source_weight_max, publisher_weight_version, entity_set,
                       event_keys, category_id, theme_id, taxonomy_basis,
                       context_hash, captured_at
                from public.event_card_contexts
            ) row_data
        ), '[]'::jsonb),
        'topic_themes', coalesce((
            select pg_catalog.jsonb_agg(
                pg_catalog.to_jsonb(row_data) order by row_data.display_name, row_data.id
            )
            from (
...
comment on function public.build_current_cluster_snapshot_v2() is
    'Builds the shared schema-v2 clustering snapshot, including exact card-birth contexts and source IDs/content hashes needed for replay verification.';

revoke all on function public.build_current_cluster_snapshot_v2()
...
comment on function public.complex_v1_capture_experiment_cluster_snapshot(uuid) is
    'Atomically captures complex_v1 schema-v2 cluster state with exact event-card contexts. A run can be captured once.';
...
comment on function public.simple_v1_capture_experiment_cluster_snapshot(uuid) is
    'Atomically captures simple_v1 schema-v2 cluster state with exact event-card contexts. A run can be captured once.';
