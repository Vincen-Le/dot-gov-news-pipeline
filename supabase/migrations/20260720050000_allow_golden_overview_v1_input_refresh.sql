...
declare
    ...
    is_v1_to_v2_refresh boolean := false;
...
begin
    ...
    if found then
        is_v1_to_v2_refresh :=
            existing_row.enrichment_version = 1
            and existing_row.prompt_version = 1
            and p_enrichment_version = 2
            and p_prompt_version = 2;

        if existing_row.input_hash <> p_input_hash
            and not is_v1_to_v2_refresh then
            raise exception
...
comment on function public.publish_golden_event_card_article_overview(
    ...
    'Atomically inserts or version-upgrades one card overview. A v1-to-v2 upgrade may refresh the source snapshot; v2 rows remain immutable.';
