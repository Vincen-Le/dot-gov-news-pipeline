...
alter table public.news_entries
    ...
    drop constraint news_entries_summary_bounded;

comment on column public.news_entries.summary is
    'Publisher-provided feed/API summary or page description. Preserved in full after markup and entity cleanup.';
comment on column public.news_entries.body_text is
    'Complete cleaned article/report text. The immutable raw response is referenced through news_backfill_run_entries.raw_artifact_key.';
comment on column public.news_entries.content_hash is
    'sha256 hex of normalized title, publisher summary, and cleaned body text; dedupe layer 2 and enrichment cache key.';
