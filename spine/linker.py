...
class Linker:
    ...
    def _new_storyline(self, row: dict, vec, t: datetime, method: str,
        ...
        # inline by design: production processes entry-by-entry, so the
        # replay pays the same per-entry latency the stream will pay
        # (optimizing that path is a production fast-follow, not a replay
        # shortcut — docs/Fast follow ups.md)
