...
Full command reference: `docs/operations/cli-cheatsheet.md`. The
[`pipeline/` guide](../pipeline/README.md) explains which local database,
engine identifier, and experiment/snapshot tables belong to `simple_v1` and
`complex_v1`.

## Your models, your quota

Embedding and LLM calls use your Cloudflare account. Model choices are env
vars (see `pipeline/shared/config.py`): `EMBEDDING_MODEL`, `ENRICHER_MODEL`,
...
Experiment history is purely local. The simple engine writes
`simple_v1_experiment_runs` and namespaced snapshots; the complex engine writes
`complex_v1_experiment_runs` and its compatibility rank table. Your runs never
leave your machine.
