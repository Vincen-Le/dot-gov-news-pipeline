...
**Architecture snapshot:** 2026-07-20
**Repository:** `dot-gov-news-pipeline`
**Status:** Infrastructure and GSA inventory reconciliation hosted-verified; bounded news-source discovery is provisioned disabled; the generalized news-source database model is implemented; curated backfill, offline simple/complex clustering, and card asset tooling are implemented; recurring discovery, complete feed inventory, source-polling workers, and the public API/UI remain planned work
...
The deployed provider resources are infrastructure scaffolding, not a complete
collection service. The hourly heartbeat is operational, discovery is guarded
by `DISCOVERY_ENABLED=false`, and there is no recurring worker that claims and
polls `news_source_fetch_state`. Until feed coverage, polling adapters,
normalization handoff, retry/DLQ operations, and freshness monitoring are
implemented and enabled, corpus growth depends on explicit backfills.
...
| Entry normalization/deduplication        | Implemented                    | `news_entries` schema, extraction/normalization in backfill + `pipeline/shared/normalize.py`    |
| Clustering, ranking, cards, topics       | Implemented (offline)          | Active `pipeline/simple`, retained `pipeline/complex`, and common `pipeline/shared` modules     |
| Clustering lab                           | Implemented                    | Operator-console lab surface (`pnpm ops lab …`) plus `complex_v1_experiment_runs` history       |
| Card thumbnails and article synthesis    | Implemented for reviewed cards | Independent lanes under `apps/image_and_synthesis_gen`; generalized card eligibility is future  |
...
apps/image_and_synthesis_gen/ Card thumbnail and article-synthesis tooling
...
Node 24 is pinned with `mise`. The root package uses pnpm 11.9.0. The Python
3.12+ `uv` environment hosts both clustering families and their pytest suite.
See `pipeline/README.md` for module ownership, registry/database setup, and
the experiment/snapshot tables selected by each engine.
...
The [database guide](database/README.md) documents clean reconstruction,
security roles, and verification. The [schema reference](database/schema-reference.md)
and [relationship guide](database/relationships.md) are the current
human-readable catalog; ordered SQL migrations remain the executable source of
truth.
...
- `pipeline/` (Python) implements two experiment families behind
  ...
  and fp16 embeddings (stored in `news_entries.embedding`, not Chroma). The
  active `simple/` spine is the path used to produce golden tables. The older
  `complex/` path runs stage 1 episodes (`complex/episodes.py`), stage 2
  storylines (`complex/storylines.py`), stage 3 event and overview cards with
  rubric rank keys (`shared/cards.py`, `compute_rank_key`), and stage 4 topic
  themes (`complex/topics.py`, nearest-centroid assignment with LLM
  adjudication against the seeded taxonomy). In stage 2, event keys and entity overlap only nominate
...
- The shared model layer (`pipeline/shared/ai.py`,
  `pipeline/shared/prompts.py`, `pipeline/shared/cache.py`, and
  `pipeline/shared/stub.py`)
...
- The operator-console clustering lab (`pnpm ops lab …`, dashboard Lab pages)
  provides corpus inspection, namespaced simple/complex experiment runs and
  immutable snapshots, storyline QA, quality metrics, and borderline
  labeling. See `pipeline/README.md` and
  `docs/operations/clustering-lab.md`.
- `apps/image_and_synthesis_gen` validates and publishes thumbnail and article
  synthesis artifacts through independent lanes. Its current trusted export
  adapter targets reviewed golden overview cards; future card types should add
  eligibility adapters rather than duplicate the asset pipelines.
...
8. **Plan drift:** the documents in `docs/archive/implementation-plans/` are useful historical artifacts but may contain stale status. Code, applied migrations, this architecture snapshot, and the runbook take precedence.

## Handoff map

Use these files rather than reconstructing context from scratch:

- `docs/architecture.md`: target architecture, current state, decisions, and phase boundaries.
- `docs/archive/implementation-plans/gsa-inventory-and-news-source-discovery-implementation-plan.md`: detailed inventory/discovery tasks, tests, and original field-level proposals.
- `docs/archive/implementation-plans/minimal-infrastructure-bootstrap-implementation-plan.md`: original foundation plan; its R2-pending status is stale.
