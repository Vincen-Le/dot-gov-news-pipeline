...
The repository currently provides the infrastructure bootstrap only:

- Supabase durable event storage.
- Cloudflare Worker, Cron Trigger, Queue, DLQ, and R2 artifact storage.
- Local persistent Chroma development service.
- Shared event contracts, tests, CI verification, and operations guidance.

Feed inventory, discovery, polling, parsing, embeddings, search, ranking,
public APIs, and the user interface are follow-up work. Do not infer that they
exist from their architectural designs.
