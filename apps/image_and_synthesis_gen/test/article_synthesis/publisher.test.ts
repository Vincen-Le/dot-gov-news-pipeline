...
} from "../../src/article_synthesis/publisher.js";
import { type ValidatedArticleOverviewV2 } from "../../src/article_synthesis/validation.js";
...
describe("article overview v2 publishing", () => {
  ...
  it("allows a v1 row with a stale input snapshot to be upgraded", () => {
    const value = validated();
    expect(() =>
      assertArticleOverviewRowsUpgradeable(
        [
  ...
  it("rejects a v2 row for a different source snapshot", () => {
    const value = validated();
    expect(() =>
      assertArticleOverviewRowsUpgradeable(
        [
