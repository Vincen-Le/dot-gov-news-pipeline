...
import { type ValidatedArticleOverviewV2 } from "./validation.js";
...
export function assertArticleOverviewRowsUpgradeable(
  ...
  for (const row of existingRows) {
    ...
    const isV1ToV2Refresh =
      row.enrichment_version === 1 &&
      row.prompt_version === 1 &&
      artifact.enrichmentVersion === 2 &&
      artifact.promptVersion === 2;
    if (row.input_hash !== artifact.inputHash && !isV1ToV2Refresh) {
      throw new Error(
        `article overview already contains a different input hash for card ${row.event_card_id}`,
      );
    }
